...
-- =====================================================================
-- 🔍 KHỐI KIỂM THỬ VÀ DEMO CHẠY THỬ TRIGGERS (CÓ THÔNG BÁO CHO CẢ VALID / INVALID)
-- =====================================================================

-- Khởi tạo biến môi trường cho kiểm toán
SET @app_client_ip = '192.168.1.55';
SET @current_official_id = 2;

-- 1. Trigger trg_dossier_check_status (BEFORE UPDATE ON dossier)
SELECT '--- 1. Kiểm thử Trigger trg_dossier_check_status ---' AS 'Kiem_Thu';
-- [VALID] (Cập nhật ghi chú của hồ sơ draft/submitted hợp lệ)
UPDATE dossier SET note = 'Đã cập nhật kiểm tra hồ sơ' WHERE dossier_id = 11;
SELECT @trigger_msg AS 'Thong_Bao_Trigger';
-- [INVALID] (Cố tình sửa hồ sơ đã được duyệt -> Bị chặn đứng ném lỗi)
-- UPDATE dossier SET note = 'Cố sửa hồ sơ đã duyệt' WHERE dossier_id = 2;

-- 2. Trigger trg_dossier_audit_status (AFTER UPDATE ON dossier)
SELECT '--- 2. Kiểm thử Trigger trg_dossier_audit_status ---' AS 'Kiem_Thu';
-- [VALID] (Kiểm duyệt viên duyệt hồ sơ ghi log kiểm toán)
UPDATE dossier SET status = 'approved', reviewed_by = 2 WHERE dossier_id = 11 AND status = 'submitted';
SELECT @trigger_msg AS 'Thong_Bao_Trigger';
SELECT * FROM audit_log WHERE table_name = 'dossier' AND record_id = 11 ORDER BY audit_log_id DESC LIMIT 1;
-- [INVALID] (Cố tình chuyển trạng thái hồ sơ sai quy trình)
-- UPDATE dossier SET status = 'draft' WHERE dossier_id = 11;

-- 3. Trigger tg_dossier_after_approve (AFTER UPDATE ON dossier)
SELECT '--- 3. Kiểm thử Trigger tg_dossier_after_approve ---' AS 'Kiem_Thu';
-- [VALID] (Duyệt hồ sơ Thương binh -> Tự cấp trợ cấp tương ứng)
UPDATE dossier SET status = 'approved', reviewed_by = 2 WHERE dossier_id = 13 AND status = 'submitted';
SELECT @trigger_msg AS 'Thong_Bao_Trigger';
SELECT * FROM citizen_allowance WHERE citizen_id = 13;

-- 4. Trigger trg_allowance_need_policy (BEFORE INSERT ON citizen_allowance)
SELECT '--- 4. Kiểm thử Trigger trg_allowance_need_policy ---' AS 'Kiem_Thu';
-- [VALID] (Cấp ví trợ cấp cho người có chính sách Thương binh active)
INSERT INTO citizen_allowance (citizen_id, regime_id, start_date, status) VALUES (1, 1, CURDATE(), 'active') ON DUPLICATE KEY UPDATE status = 'active';
SELECT @trigger_msg AS 'Thong_Bao_Trigger';
-- [INVALID] (Cố tình cấp trực tiếp trợ cấp cho người thường ID 24 -> Chặn)
-- INSERT INTO citizen_allowance (citizen_id, regime_id, start_date, status) VALUES (24, 1, CURDATE(), 'active');

-- 5. Trigger trg_allowance_need_policy_update (BEFORE UPDATE ON citizen_allowance)
SELECT '--- 5. Kiểm thử Trigger trg_allowance_need_policy_update ---' AS 'Kiem_Thu';
-- [VALID] (Cập nhật ngày bắt đầu trợ cấp hợp lệ)
UPDATE citizen_allowance SET start_date = '2026-05-15' WHERE citizen_id = 1 AND regime_id = 1;
SELECT @trigger_msg AS 'Thong_Bao_Trigger';
-- [INVALID] (Cố cập nhật ví trợ cấp sang cho người thường)
-- UPDATE citizen_allowance SET status = 'active', citizen_id = 24 WHERE citizen_id = 1 AND regime_id = 1;

-- 6. Trigger trg_policy_expire_suspend_allowance (AFTER UPDATE ON object_mapping)
SELECT '--- 6. Kiểm thử Trigger trg_policy_expire_suspend_allowance ---' AS 'Kiem_Thu';
-- [VALID] (Ngưng diện chính sách của công dân 12 -> Đình chỉ trợ cấp liên đới)
UPDATE object_mapping SET status = 'paused' WHERE citizen_id = 12 AND object_id = 6;
SELECT @trigger_msg AS 'Thong_Bao_Trigger';
SELECT * FROM citizen_allowance WHERE citizen_id = 12;

-- 7. Trigger tg_audit_citizen_changes (AFTER UPDATE ON citizen)
SELECT '--- 7. Kiểm thử Trigger tg_audit_citizen_changes ---' AS 'Kiem_Thu';
-- [VALID] (Cán bộ sửa địa chỉ chi tiết công dân -> Tự ghi audit_log)
